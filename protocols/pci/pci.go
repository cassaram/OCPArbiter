...
func (p *PCIProtocol) StartServices() {
	// Initialize state
	p.state.Set(PCI_START)

	// Start services
	go p.protocolStateService()
	go p.protocolService()
	go p.rxService()
	go p.txService()
}
...
func (p *PCIProtocol) protocolStateService() {
	for {
		// Check modes
		switch p.state.Get() {
		case PCI_START:
...
func (p *PCIProtocol) protocolService() {
	for {
